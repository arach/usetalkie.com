...
| | Talkie | DictaFlow |
|---|---|---|
| Pricing | See current plans | $7/mo or $69/year for the consumer plan |
| On-device transcription | Yes — Parakeet v3 (Mac), Apple Speech (iOS) | Yes in Local Offline; cloud modes are also available |
| Storage | Local-first SQLite on your Mac | App-managed; processing depends on the selected mode |
| CLI for scripts & agents | Yes — `@talkie/cli` | No |
| Dictate into AI agents | Yes — e.g. Claude Code | Not a focus |
| Cross-device | Mac, iPhone, Apple Watch (iCloud) | Windows, Mac, iPhone |
| Windows support | No (Apple-only) | Yes |
| Cloud AI formatting | No (fully local) | Yes, in cloud modes |
